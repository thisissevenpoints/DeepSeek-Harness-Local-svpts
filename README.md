...
- 界面右上角 ⏻ 按钮：确认后完全退出（前端 + 后台）；其左侧 ⟳ 按钮：确认后彻底重启（前端 + 后台，整个应用 relaunch）
...
2. 手机连同一 WiFi，浏览器访问 `http://<本机IP>:3280`（`0.0.0.0:3280` 反向代理 → 本机 dsh 端口，HTTP + WebSocket 双转发）
...
- **生命周期**：关窗 = 回到托盘、后台继续运行；托盘右键或界面 ⏻ 按钮 = 完全退出（前端 + 后台）；⟳ 按钮 = 彻底重启（确认后整个应用 relaunch，新进程重跑看门狗并重新拉起后台）
- **自绘顶栏**（无边框窗口）：`⟳ 彻底重启 / ⏻ 完全退出 / ─ 最小化 / ⛶ 最大化-还原 / ✕ 关闭窗口`（自右向左，整条可拖拽移动窗口）
- **底部状态栏**：后端在线状态（每 5 秒探测）+ 端口（启动时实测选定，自动跳过 Windows 保留段）+ 工作区路径 + 局域网开关 + **本机局域网 IP** + 版本
...
- `desktop/main.cjs`：单一常驻托盘 Electron 应用。看门狗逻辑（后台唯一 owner：健康巡检、子进程存活判据防误杀慢启动、崩溃自动重启、`watchdog.stop` 文件信号停止）+ 窗口逻辑（loading 页 → 真实 UI → 页面注入顶栏按钮；重启 = `app.relaunch` 整个应用，`--dsh-restarted` 标记供新进程识别）

...
把 dsh Web UI（`http://127.0.0.1:<启动时实测选定的端口>`，候选 3180/2180/4180/6180/8180）包进一个 **常驻系统托盘的 Electron 应用**：应用本身同时是后台 dsh 服务的唯一 owner（看门狗）与桌面壳（按需开窗）。
...
| 底部状态栏 | `● 后端在线/离线`（5 秒自动探测）+ 端口 + 工作区路径 + 局域网开关状态 + **本机局域网 IP** + 版本 |
| `🌐 局域网`（功能栏） | 局域网转发开关：开启后 `0.0.0.0:3280` 反向代理（HTTP+WS）到本机 dsh；未授权设备见申请页 → 电脑端确认 → 持久 Cookie 授权（`lan-auth.json`，删除即撤销）；默认关闭 |
...
- **端口被占/被系统保留**：启动时按候选表实测选定可用端口（跳过 Windows 动态保留段）；候选端口上已有 HTTP 服务时直接复用（不重复启动）；停止脚本会停掉候选端口上的一切实例（明确停止即停一切）。

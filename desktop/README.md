...
| 窗口右上角按钮 | `⏻ 完全退出`（确认后停前后端）、`✕ 关闭窗口`（回托盘）、`⛶ 最大化/还原`、`─ 最小化`（无边框自绘顶栏，整条可拖动） |
| 顶栏下方 `📁 打开会话存档` | 打开 `dsh-home\sessions` 文件夹（功能按钮区，预备扩展） |
| 底部状态栏 | `● 后端在线/离线`（5 秒自动探测）+ 端口 + 工作区路径 + 版本 |
...
- 无开机自启、无更新检查、无打包；
- 仅本地 127.0.0.1，不对外暴露；
- 无边框窗口的拖动区域在顶栏；关闭窗口=回托盘（彻底退出用 ⏻ 或停止脚本）。

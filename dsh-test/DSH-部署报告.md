...
> **2026-08-16 迁移记录**：整体迁移至 `D:\alpha\DeepSeek-Harness-Local\`。
> - 源码：`C:\Users\iamse\dev\deepseek-harness` → `D:\alpha\DeepSeek-Harness-Local\deepseek-harness`（全新 clone 后重装重建）
> - Harness home：`C:\Users\iamse\.dsh` → `D:\alpha\DeepSeek-Harness-Local\dsh-home`（已设置用户环境变量 `DSH_HOME` 指向此处）
> - 测试区：`C:\Users\iamse\dsh-test` → `D:\alpha\DeepSeek-Harness-Local\dsh-test`
...
源码位置：`D:\alpha\DeepSeek-Harness-Local\deepseek-harness`（官方 README "Run from source" 流程，未做任何改动）
...
配置（`D:\alpha\DeepSeek-Harness-Local\dsh-home\settings.yaml`，遵循 `llm-pi-ai` 与 `agent-default-model` 官方 schema）：
...
`D:\alpha\DeepSeek-Harness-Local\dsh-home\.env`：`OLLAMA_PLACEHOLDER_KEY=ollama`（占位值，非真实密钥）。

## 3. 启动 / 停止 / 重启

| 操作 | 命令 |
|---|---|
| 启动 | `cd D:\alpha\DeepSeek-Harness-Local\deepseek-harness && pnpm dsh web` |
...
| 升级 | `cd D:\alpha\DeepSeek-Harness-Local\deepseek-harness && git pull && pnpm install && pnpm run build` 后重启 |
...
| 3 | 配置模型并完成真实任务往返（独立临时目录创建 hello.txt 写入 Hello） | ✅ 通过（有本地模型注意事项，见下） | 独立临时目录（现位于 `D:\alpha\DeepSeek-Harness-Local\dsh-test\hello-workspace-760`），agent（qwen3:8b）创建 `hello.txt`，内容精确为 `Hello`（5 字节，已 cat 验证）；headless 运行 exit 0、turn reason=completed。另完成 PONG 纯文本往返验证 |
| 4 | 凭证存储位置与权限 | ✅ 已核实 | 凭证目录 `D:\alpha\DeepSeek-Harness-Local\dsh-home\`（`DSH_HOME` 已设为用户环境变量）。`.credentials.yaml` 在 UI 保存 Key 后生成；settings.yaml/.env 权限 644。部署全程未产生任何真实密钥 |
| 5 | 退出与重启方式 | ✅ 已记录 | 见第 3 节 |

**验收 3 的本地模型注意事项（重要）**：
- 小型本地模型会被 dsh 默认注入的"runtime context"尾随用户消息带偏（把环境说明当成本次指令）。已验证的解决方案：headless 运行时附加补丁 `--patch D:\alpha\DeepSeek-Harness-Local\dsh-test\no-runtime-context.yml`（关闭 `system-prompt` 的 `includeRuntimeContext`）。DeepSeek 云端 API 等强模型无此问题，Web UI 默认配置无需改动；如需在 Web UI 长期使用本地小模型，可将该补丁内容合并进 `D:\alpha\DeepSeek-Harness-Local\dsh-home\profiles\web\cordis.patch.yml`。
...
```sh
# 升级
cd D:\alpha\DeepSeek-Harness-Local\deepseek-harness && git pull && pnpm install && pnpm run build
# 启动（前台，Ctrl+C 停止）
cd D:\alpha\DeepSeek-Harness-Local\deepseek-harness && pnpm dsh web
# 查看监听进程
...
# 日志与数据
#   会话记录：D:\alpha\DeepSeek-Harness-Local\dsh-home\sessions\（zstd 压缩 JSONL）
#   配置：     D:\alpha\DeepSeek-Harness-Local\dsh-home\settings.yaml（热生效）、.env（启动加载）、profiles\*\cordis.patch.yml
#   Ollama 模型：C:\OllamaModels\（按用户选择未迁移）；日志：C:\Users\iamse\AppData\Local\Ollama\
```
...
5. **下一步建议**：
   ...
   - UI 中 "Choose workspace" 选择你要用的项目目录（建议先用 `D:\alpha\DeepSeek-Harness-Local\dsh-test\hello-workspace-760` 试跑）；
...
- 位置：`D:\alpha\DeepSeek-Harness-Local\desktop\`，启动：`cd desktop && npm start`。

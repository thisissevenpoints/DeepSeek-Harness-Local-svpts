...
rem 3) 端口占用提示：已有服务时壳会自动复用，不会误杀
rem    端口由壳启动时实测选定（候选 3180 2180 4180 6180 8180，自动跳过 Windows 动态保留段）
for %%P in (3180 2180 4180 6180 8180) do (
    %SystemRoot%\System32\netstat.exe -ano | %SystemRoot%\System32\findstr.exe /C:":%%P " | %SystemRoot%\System32\findstr.exe /C:"LISTENING" >nul 2>nul
    if not errorlevel 1 echo [提示] 检测到端口 %%P 已有服务在运行，壳将直接复用。
)
